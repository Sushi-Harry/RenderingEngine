...
Model::Model(const std::string &PATH, const std::string &SHADER_PATH_VS,
             const std::string &SHADER_PATH_FS, std::string NAME) {
    isSelected = false;
    LoadModel(PATH);
    this->NAME = NAME;
    ModelShader = new Shader(SHADER_PATH_VS.c_str(), SHADER_PATH_FS.c_str());
    SELECTION_SHADER = new Shader("Essentials/ImpShaders/selectionMode.vs",
                                  "Essentials/ImpShaders/selectionMode.fs");
}

void Model::SetID(int ID) {
    this->ID = ID;
    r = (ID / 255.0f);
    SELECTION_SHADER->setVec4("PickingColor", glm::vec4(r, 0.0f, 0.0f, 0.0f));
}

void Model::Draw(glm::mat4 PROJECTION, glm::mat4 VIEW_MATRIX,
                 glm::vec3 VIEW_POS, float ScaleFactor) const{
...
/*
void Draw(glm::mat4 PROJECTION, glm::mat4 VIEW_MATRIX, glm::vec3 VIEW_POSITION, float scale) const;
    void Draw_NO_SHADER_USE(glm::mat4 PROJECTION, glm::mat4 VIEW_MATRIX, glm::vec3 VIEW_POSITION, float scale, Shader& shader) const;
*/

void Model::Draw_NO_SHADER_USE(glm::mat4 PROJECTION, glm::mat4 VIEW_MATRIX,
...
void Model::Draw_SELECTIONMODE(glm::mat4 PROJECTION, glm::mat4 VIEW_MATRIX,
                               glm::vec3 VIEW_POS) {
    for (unsigned int i = 0; i < meshes.size(); i++) {
...
void Model::scaleModel(float ScaleFactor, glm::mat4 &model_matrix) const{
